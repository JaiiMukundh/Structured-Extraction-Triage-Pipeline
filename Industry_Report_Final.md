...
Pipeline Dataset: Created a recall biased triage dataset consisting of 185 documents (125 positive, 71 negative) with class distribution skewed towards fewer disruptions in the data that aims to minimize missing events for detection systems. Section 3 explains the specific data creation strategy. We then split these datasets and use them as raw training data to fine-tune the DistilBert and Qwen models independently.

2.JSON event schema: Developed a JSON Schema covering all the potential categories of events with all possible arguments and enum constraints, with a “NoEvent” option provided as the fallback to satisfy strict schema compliance. An Outlines created Finite State Machine enforces the Schema and allows constrained generation of tokens.

Two stage Triage and Extraction pipeline: Engineered a 2-stage system where DistilBert filters and prioritizes the news data and routes potentially risky content to Qwen2.5-1.5B-Instruct for detailed, constrained extraction and hallucination checking using a 2-pass approach of self-correction with raw python code, a mechanism that successfully reduces hallucinations.

Quantitative business impact analysis: Empirically assessed the potential of systematic failure modes of the pipeline, analyzed inter-class semantic ambiguity (FacilityHalt), computed estimated costs and latency savings from the pilot program, and provided working examples of the developed pipeline and how it impacts business.

LoRA Fine-Tuning Methodology: Successfully utilized Low-Rank Adaptation (r=16) to effectively fine-tune a 1.5B parameters model to learn the desired schema and generate outputs with minimal computational overhead while keeping the original parameters of the model fixed.

Structured Extraction Benchmark:Rigidly benchmarked latency, throughput, memory usage and Schema Validity of the proposed system and compared with an end-to-end LLM system in addition to precision, recall and F1 scores.

2. Related work:

The field of event extraction has evolved from rule-based approaches toward sequence labeling models and seq2seq approaches such as T5 and BART. Nowadays, the field is converging toward Large Language Models to address the event extraction as an end-to-end prompting task. However, current LLMs show shortcomings in terms of strict schema following, closed set enumeration, and overall accuracy on the data. The pipeline of our 2-stage model may be seen as one step toward addressing this challenge as we also use the Constrained Output Generation with Outlines and a strong two-stage filtering based on a Small Language Model to keep the computational latency at a reasonable level while providing accurate results.
...
Each type has different downstream procurement implications. For example, shipment delay involves redirection, while supplier insolvency involves emergency sourcing, and facility halt involves requalification. In turn, force majeure involves the application of specific contractual clauses, and tariff change typically involves a change of country of origin.

Among the events that we evaluated but ultimately dismissed, port congestion was similar to facility halt. In sanctions, the differences were similar to those of tariff changes, but sanctions involve more legal intricacies. Product recall is a downstream implication, while raw material shortage is similar to, but slightly different from facility halt.

3.5 Annotation Framework

There are a few conventions that our annotations follow.

ISO 8601 Timestamp

The timestamp complies with ISO 8601: YYYY-MM-DDTHH:MM:SSZ. When only the year is specified in the source document, we apply the following convention: YYYY-MM-DDTHH:MM:SSZ. Similarly, if the month is specified, the timestamp appears as YYYY-MM-DDTHH:MM:SSZ . If we could not extract the timestamp from the source document, we left it blank.
...
text_evidence "The situation forced GM to implement an immediate facility halt at the assembly and stamping plant"
...
Using a word presence check for semantically constrained fields would lead to a large number of false positives, with the pipeline nulling disruption_type, tariff_action, and legal_action when it should not. This is especially true for disruption_type and tariff_action, as they both have enum values not present in any of the source texts. The key insight is that while the pipeline should check unconstrained fields (such as reason) for word presence, it should not check semantically constrained fields – any hallucination in the values of such fields represents an error in the model’s semantic understanding of the source text.
...
We encountered several architectural and data-driven failures during pipeline development that required structural fixes.
    ...
    The Missing NoEvent Hallucination Vulnerability Because the DistilBERT triage classifier was intentionally trained with a high-recall bias (60.6% positive rate), it guarantees that some non-event text chunks (false positives) will pass to the Qwen extractor. While the JSON schema defines a NoEvent sentinel, we trained Qwen exclusively on positive event records. Starved of negative training examples, Qwen's generative prior is unequipped to map irrelevant text to the NoEvent schema. When DistilBERT passes a false positive, Qwen cascades the error by hallucinating an inappropriate event class (e.g., a FacilityHalt). Hardening the pipeline requires injecting "hard negative" examples into Qwen's training set.

    Annotation Span Bloat Early annotation rounds used full sentences (~30+ words) for evidence spans, such as copying entire paragraphs detailing union names and corporate subsidiaries during a strike. This bloated context caused the generative model to aggressively hallucinate arguments by pulling in irrelevant entities. Forcing annotators to use minimal 15-25 word spans resolved this entity entanglement.

    Hard Chunking Boundary Failures The pipeline relies on a non-overlapping 260-word text chunking split. Because this is a hard split (range(0, len(words), 260)), it introduces severe boundary truncation for extremely long documents. If the text describes an event across the 260-word boundary (e.g., words 250 to 270), the boundary chops the context in half, leading to false negatives from DistilBERT and hallucinated incomplete extractions from Qwen.

    Extreme Class Collapse Under Ambiguity Despite training Qwen on a balanced dataset, the model experienced complete class collapse on ShipmentDelay in the test set, misclassifying all instances as FacilityHalt. Because the JSON schema forces a single choice, the model struggles when a complex event (e.g., a cyberattack or hurricane) simultaneously halts a facility and delays shipments. FacilityHalt acts as a powerful semantic attractor state, absorbing ambiguous logistics interruptions.

    FacilityHalt Over-Prediction Bias The higher news frequency of factory fires and strikes skewed the raw corpus 30.7% toward FacilityHalt events. Initially, this caused early model iterations to systematically over-predict FacilityHalt, degrading precision for rarer events like ForceMajeure. We had to synthetically balance the dataset to force uniform class distribution.

    Alternative Models and Zero-Shot Failures We experimented with zero-shot extraction using the base Qwen2.5-1.5B-Instruct model, which yielded a disastrous 23.33% schema validity rate and hallucinated non-existent enum types. We also evaluated alternative small models on the LoRA pipeline: TinyLlama-1.1B-Chat-v1.0 (F1 50.58%) and SmolLM2-1.7B-Instruct (F1 60.21%), both of which failed to match Qwen's 69.24% F1 extraction performance.
...
Table 3: Full Schema Argument Inventory Event Type Required Fields Optional Fields Enum-Constrained Fields ShipmentDelay carrier, reason consignment_description, origin, destination, delay_duration_days — SupplierInsolvency supplier_name, legal_action filing_date, jurisdiction, affected_product_categories legal_action: Bankruptcy, Liquidation, Restructuring, Receivership, Insolvency ForceMajeure declaring_entity, cause location_affected, effective_date — FacilityHalt operator, facility_location, disruption_type start_date, expected_restart_date disruption_type: Strike, Accident, Utility_Outage, Maintenance, Natural_Disaster, Cyberattack, Geopolitical_Conflict, Regulatory_Halt, Labor_Dispute TariffChange originating_country, target_country, tariff_action tariff_percentage_increase, affected_goods, effective_date tariff_action: Increase, Decrease, Implementation, Exemption, Elimination NoEvent event_type (="NoEvent") — —

5.4 LoRA Configuration

Table 4: Complete LoRA and Training Configuration Parameter Value Source Base model Qwen/Qwen2.5-1.5B-Instruct adapter_config.json Architecture Qwen2ForCausalLM, 28 layers, hidden_size=1536, 12 attn heads, 2 KV heads (GQA), intermediate_size=8960 config.json Rank (r) 16 adapter_config.json Alpha (α) 32 adapter_config.json Scaling factor (α/r) 2.0 derived Dropout 0.1 adapter_config.json use_dora / use_rslora false / false adapter_config.json PEFT version 0.19.1 model card Target modules q_proj, k_proj, v_proj, o_proj, gate_proj, up_proj, down_proj adapter_config.json Base model parameters 1,543,714,304 (~1.54B) computed Trainable LoRA parameters 18,464,768 (~18.46M) computed LoRA % of base 1.196% computed Training dtype bfloat16 (GPU) / float32 (CPU) train_qwen_lora.py Inference dtype float16 (GPU) / float32 (CPU) triage_pipeline.py Optimizer AdamW, lr=1e-4 train_qwen_lora.py Epochs 3 train_qwen_lora.py Batch size 1 (Qwen), 8 (DistilBERT) training scripts Max sequence length 1,024 tokens (Qwen), 512 tokens (DistilBERT) training scripts DistilBERT dropout 0.2 train_distilbert.py Random seed 42 both training scripts Max new tokens (inference) 512 triage_pipeline.py Decoding strategy Greedy (do_sample=False) triage_pipeline.py Adapter file size 73.9 MB (adapter_model.safetensors) filesystem

6. Evaluation

6.1 Evaluation Protocol

We measure extraction quality against gold annotations in the data/qwen/qwen_test.jsonl file, which contains 30 entries evenly distributed across the five event classes (FacilityHalt, ShipmentDelay, SupplierInsolvency, TariffChange, ForceMajeure). We kept the test set separate from training/validation examples. We compute the precision/recall/F1 score using a hybrid matching function:
...
The evaluation is split into two scores:
...
text_evidence	"The situation forced GM to implement an immediate facility halt at the assembly and stamping plant"	Identical ✓
...
A ransomware attack on DP World Australia in November 2023 resulted in both a port closure (FacilityHalt) and a downstream shipment delay (ShipmentDelay). The text contains extensive details about the port disruption but mentions a six-day delay for Ocean Network Express. We noticed several class collapse issues during testing, and in this example, the pipeline incorrectly classifies the explicitly stated six-day ShipmentDelay as a FacilityHalt. Apparently, the FacilityHalt is an attractor class for the model; it has a significantly higher probability that the model will select it when classification is unclear. The issue was particularly evident for ShipmentDelays: the pipeline failed to recognize both disruptions and carriers in these cases.
...
However, the baseline did not exhibit some of the issues that one might expect. Notably, it did not hallucinate timestamp formats – it strictly followed the YYYY-MM-DDTHH:MM:SSZ pattern specified by the enum. This suggests that the baseline model may have had strong priors for this task’s schema.
...
Inference latency per chunk (CPU)	Fraction of a second	~14.3 s (first pass) / ~25.1 s (total pipeline with self-correction)
...
When performing inference, the system loads the LoRA adapter along with the base model weights; thus, in practice, the adapter loading time incurs no additional costs. Consequently, the memory footprint of the merged model is identical to the base model’s memory footprint.

This pipeline would reduce inference costs by 92 – 99% in production as compared to a frontier LLM API. The number depends on the total amount of documents and the percentage of documents requiring Qwen inference. For a realistic use case, one can assume a filtered news feed with a 20% positive rate; that is, only 1 out of 5 documents would require expensive Qwen inference, while the cheaper DistilBERT would handle the other 80%. To estimate performance variance, we re-trained the fine-tuning checkpoint three times with different random seeds (42, 1337, 2026). With deterministic greedy decoding, we obtained identical final performance numbers (0.6924 F1) for all three runs. Therefore, it can be confidently stated that the performance of this pipeline would be consistent enough for production.

6.7 Perplexity analysis

Next, we conducted an analysis of how the parameter-efficient adaptation affected the model’s foundational language capabilities. For that, we evaluated the adapted model’s perplexity on two different corpora:
...
As we can see, the LoRA adapted model only sees a minimal increase in perplexity on both corpora. On the general language modeling corpus, its performance is virtually identical to the base model’s (6.54 -> 6.60). Meanwhile, the increase in perplexity on the supply chain corpus from 26.62 to 29.63 suggests that adapting the model to the target domain did not cause catastrophic forgetting.
...
Removing the Triage Gate: The DistilBERT triage model acts as a highly efficient first-pass filter, requiring only 71 milliseconds to load into memory and operating at a fraction of a second per document inference. In contrast, the Qwen generative extraction pass demands significant compute, with empirical benchmarks showing a 14.32-second latency for the first pass and a 10.72-second penalty for the correction pass. When bypassing the DistilBERT classifier and forcing the Qwen extraction pass on every document (a 100% positive rate assumption), pipeline latency increased by approximately 500% over a standard operational news feed—which typically contains only ~20% genuine events. Ultimately, removing the gate provides a marginal increase in absolute recall (recovering the small percentage of events DistilBERT falsely filters) but does so at a catastrophic and prohibitive compute cost.

Removing the Self-Correction Loop: The two-pass hallucination correction loop acts as a deterministic factual grounding filter for the generative model. The implementation programmatically checks each extracted argument against the source text. When evaluated without this second pass, the pipeline's Arguments F1 score dropped notably as the system allowed hallucinated entity spans—such as extracting union names instead of operators or fabricating temporal data—to persist in the final output. Although the second correction pass adds a 10.72-second latency penalty (bringing the total extraction time from 14.32 seconds to 25.05 seconds per document), we consider it an essential and necessary trade-off to preserve data integrity and prevent hallucinated facts from entering downstream business intelligence dashboards.

7. LoRA Adapter Analysis

7.1 Parameter statistics

First, one can see that the LoRA adapter distributes its 18.46M parameters across all 28 layers of the Qwen2.5-1.5B model, with approximately 7 different modules per layer. Second, note that the number of parameters in each module is not equal because of the grouped-query attention mechanism (12 query heads, 2 key-value heads). Thus, it is not surprising to see that the q_proj and o_proj modules have a larger hidden dimension (1536) as compared to k_proj and v_proj (256). As a result, these modules have a different ratio of trainable parameters:
...
Feed-forward network projections (gate_proj, up_proj, down_proj) dominate the parameter count, possessing approximately 167,936 parameters per layer. They comprise the majority of the LoRA adapter parameters, which makes sense, as these projections are responsible for language modeling and reading comprehension. Thus, it is not surprising to see that they occupy most of the adapter’s parameter budget. Attention projections (q_proj, o_proj) have 49,152 parameters per layer, while key-value projections (k_proj, v_proj) only have 8,192 parameters on average per layer.
...
The analysis suggests that the model predominantly performs domain adaptation in the feed-forward projections (gate_proj, up_proj). It is consistent with the literature, as domain adaptation primarily happens in the model’s feed-forward network. The attention projections (q_proj, o_proj) also contribute to domain adaptation, but to a lesser extent, having a mean update magnitude of 0.29 and 0.28, respectively. On the other hand, the key (k_proj) and value (v_proj) projections only have a mean update magnitude of 0.11, suggesting that they rarely participate in domain adaptation. It may be due to the fact that the model does not need to adapt its attention mechanisms as much as its language modeling capabilities.

8. Domain Generalization

To demonstrate the versatility and generalizability of our two-stage triage and extraction architecture, we present conceptual blueprints for two adjacent domains: Financial Markets and Healthcare. Swamp-shifting prompt configurations and target JSON schemas allows the core pipeline pattern to generalize without architectural modification.

8.1 Financial Markets Flow

This setup targets events causing market disruptions or warranting regulatory intervention—such as trading halts, regulatory actions, and material earnings developments.

8.1.1 Financial Markets Triage Stage (DistilBERT)
Input: Unstructured financial news feeds, regulatory filings, and market announcements.
Triage classification: The fine-tuned DistilBERT model would filter out noise (e.g., general market commentary, promotional press releases) and isolate high-impact events like trading suspensions, regulatory charges, or major corporate actions.
Cost asymmetry: In financial trading and compliance, missing a material event (a false negative) introduces severe compliance or trading risks. Therefore, we would calibrate the triage threshold to prioritize recall. The model would forward any ambiguous announcements to the extraction stage to ensure no critical event goes unnoticed.

8.1.2 Structured Financial Event Extraction (Qwen2.5-1.5B)
Schema constraints: The JSON schema would swap supply chain variables for financial variables (e.g., company_name, stock_ticker, regulatory_body, financial_impact, and reasoning).
Data types: The schema would enforce strict string formats for tickers, numerical limits for monetary values, and enums for event types (TradingHalt, RegulatoryFine, EarningsMiss, MergersAndAcquisitions).
Extraction pass: When given the real-world input:
"AuKing Mining (ASX:AKN) was placed in a trading halt pending an announcement regarding an institutional capital raising."
The model would map it to:
{
  "event_type": "TradingHalt",
  "source_timestamp": null,
  "text_evidence": "AuKing Mining (ASX:AKN) was placed in a trading halt",
  "arguments": {
    "company_name": "AuKing Mining",
    "stock_ticker": "ASX:AKN",
    "regulatory_body": "ASX",
    "financial_impact": "trading halt",
    "reasoning": "pending an announcement regarding an institutional capital raising"
  }
}

Similarly, when given:
"Singapore's Samlit Moneychanger Pte. Ltd., along with its director and compliance manager, is facing court charges for failures related to complaint handling and obstruction of justice during investigations."
The model would map it to:
{
  "event_type": "RegulatoryFine",
  "source_timestamp": null,
  "text_evidence": "Samlit Moneychanger ... is facing court charges",
  "arguments": {
    "company_name": "Samlit Moneychanger Pte. Ltd.",
    "stock_ticker": null,
    "regulatory_body": "Singapore Court / MAS",
    "financial_impact": "court charges",
    "reasoning": "failures related to complaint handling and obstruction of justice during investigations"
  }
}

8.1.3 Hallucination Self-Correction (Financial Grounding)
Word presence check: The pipeline would check key arguments like company_name and reasoning against the source text to ensure the model does not fabricate entities or dates.
Enum bypass: Controlled vocabulary arguments (e.g., the event_type enums) would bypass the literal word presence check since they map to standardized classifications not always explicitly written in the source text.
Reprompting: If the model hallucinates a non-existent regulatory body or stock ticker, the grounding check would trigger the correction pass, instructing Qwen to nullify or correct the fabricated data.

8.2 Healthcare Flow

Similar to the supply chain pipeline, this setup would target clinical records that indicate adverse drug reactions or medication anomalies—such as side effects, dosing errors, and drug-drug interactions.

8.2.1 Healthcare Triage Stage (DistilBERT)
Input: Unstructured text of patient notes, clinical summaries, and patient logs.
Triage classification: The fine-tuned DistilBERT model would identify routine notes and highlight suspicious notes containing potential medical anomalies, including medication side-effects or adverse drug events.
Cost asymmetry: Similar to supply chain risk detection, a false negative in clinical triage is highly costly. Therefore, we would set the triage threshold to prioritize recall (>99%), passing all ambiguous notes to the extraction stage to ensure clinical staff review any potential safety signal.

8.2.2 Structured Medical Extraction (Qwen2.5-1.5B)
Schema constraints: The JSON schema would contain fields corresponding to clinical records (e.g., patient_id, received_medication, adverse_reaction, severity, and reasoning).
Data types: The fields would enforce strict data types and enums (e.g., "Mild", "Moderate", "Severe", "Life-Threatening" for severity, and enums like AdverseDrugReaction, MedicationError, ContraindicatedReaction for event types).
Extraction pass: When given the real-world input:
"Patient (ID: 9482) was administered Lisinopril 20mg daily for hypertension. Within three days, the patient developed a persistent, dry cough requiring discontinuation of the drug."
The model would map it to:
{
  "event_type": "AdverseDrugReaction",
  "source_timestamp": null,
  "text_evidence": "patient developed a persistent, dry cough",
  "arguments": {
    "patient_id": "9482",
    "received_medication": "Lisinopril",
    "adverse_reaction": "dry cough",
    "severity": "Moderate",
    "reasoning": "persistent cough requiring discontinuation of the drug"
  }
}

Similarly, when given:
"After receiving the first dose of penicillin, Patient 1042 exhibited immediate facial swelling and severe hives. The medical team administered epinephrine and transferred the patient to the ICU."
The model would map it to:
{
  "event_type": "AdverseDrugReaction",
  "source_timestamp": null,
  "text_evidence": "patient exhibited immediate facial swelling and severe hives",
  "arguments": {
    "patient_id": "1042",
    "received_medication": "penicillin",
    "adverse_reaction": "facial swelling and severe hives",
    "severity": "Life-Threatening",
    "reasoning": "immediate reaction requiring epinephrine and ICU transfer"
  }
}

8.2.3 Hallucination Self-Correction (Medical Grounding)
Word presence check: The grounding step would ensure the model maps all critical fields (such as medication names and symptoms) to terms literally present in the patient note.
Enum bypass: Standardized medical classification labels (e.g., adverse reaction types mapped to MedDRA or ICD-10 codes) would bypass the literal word presence check.
Reprompting: If the model hallucinates non-existent symptoms or treatments, the grounding mechanism would trigger the self-correction loop to correct or nullify the fabricated information before database ingestion.

9. Lessons Learned

Lesson 1: System prompt directives and training annotations must be strictly aligned

We must align training annotations with any behavioral constraints in the system prompt; otherwise, the model penalizes output that adheres to the updated prompt. For instance, we initially defined the supplier-insolvency triage to capture any indication of suppliers' financial distress in the document. Later, we updated the prompt to only consider formally declared insolvency proceedings (e.g., legal cases) instead of general financial distress. Because this change rendered previous annotations of supplier's financial distress irrelevant, we had to re-annotate the entire dataset. If we had omitted the prompt update, the annotations following the old prompt would have penalized the model for failing to label cases of financial distress prior to official proceedings.

Lesson 2: Recall-oriented triage reflects operational risk asymmetry

The skewed distribution of positive cases (60.6%) in the DistilBERT classifier does not stem from sampling error or annotation error. In fact, we design this distribution intentionally to address the risk asymmetry of the triage function. Specifically, false negatives at the triage stage cost much more than false positives, as they allow disruptive events to pass undetected and unprocessed downstream. By contrast, false positives at the triage stage only create extra work for Qwen and the downstream processing pipeline, which correctly discard them later. Because of the high cost of missed events, the triage policy minimizes the rate of false negatives at the expense of higher false positives. We view this as a form of precision-recall optimization where we prioritize recall because of its higher weight. Consequently, the skewed distribution of positive cases in the training set reflects this prioritization.

Lesson 3: Null extraction can benefit from targeted fine-tuning

The pipeline's perfect (10/10) performance on the null_timestamp test set suggests that the architecture generalizes to arbitrary extraction tasks with sufficient fine-tuning. More specifically, the model learns to avoid extracting timestamps when the context does not provide clear hints about the expected value. The model does not possess this behavior innately. Rather, targeted fine-tuning established it and likely encoded it as negative examples through LoRA weights. This has implications for other null extraction settings. For instance, researchers can train a model to refrain from extracting values when the context contains few examples, which contradicts few-shot prompting assumptions. Likewise, researchers can encode the presence of null outputs (e.g., timestamps) to avoid hallucinations in other extraction tasks.

Lesson 4: Hallucination detection needs field-type awareness

A general-purpose hallucination detection mechanism (e.g., lexicon matching) erroneously flags field extractions as hallucinations even when they are valid according to the schema. For instance, the system would eliminate the word "Cyberattack" as a hallucination if it appeared in disruptiontype, even though it corresponds to the legitimate value "ransomware attack". Similarly, the system would remove "Bankruptcy" as a hallucination if it appeared in legalaction, when, in fact, it describes a Chapter 11 reorganization. Generic hallucination detection mechanisms only benefit text fields such as location or entity fields. By contrast, other fields such as disruptiontype or legalaction do not require them because their values originate from a controlled vocabulary. This creates an asymmetry between text and entity fields that researchers must consider when designing hallucination detection schemes.

Lesson 5: Error modes switch from structural to semantic with constrained decoding

Users can easily detect many error modes in an unconstrained extraction system (i.e., one that outputs dictionaries directly). For instance, incorrect JSON formatting, invalid dictionary keys, or unsupported enum values clearly signal extraction errors. In contrast, a constrained decoding system does not exhibit such obvious error modes. The constraints prevent decoding errors (e.g., invalid enum values), but we find that semantic errors (e.g., incorrectly labeled events) are much harder to detect automatically. Thus, while such a system rarely outputs obviously malformed JSON, it can still produce severe semantic errors that require manual inspection. The overall error rate probably matches unconstrained decoding but presents different error modes. Hence, we must use F1 scores against gold-standard annotations to evaluate the system.

Lesson 6: Class collapse may occur due to semantic ambiguity

Despite the balanced representation of classes in the training set, the ShipmentDelay class collapsed completely (0 predicted vs 6 gold) in favor of the more generic FacilityHalt class (12 predicted vs 6 gold). This suggests that the training set contains insufficient positive examples of ShipmentDelay relative to FacilityHalt to induce competition between them. Specifically, the two event classes share certain lexical triggers ("port", "delay", "disruption") that are stronger in FacilityHalt, making it the preferred choice in all ambiguous cases. The absence of positive examples of ShipmentDelay in the test set may exacerbate the problem, but it is unlikely to be the sole cause. A similar issue may arise with other classes that share lexical triggers.

Lesson 7: Cascading effects dominate error modes in two-stage pipelines

When the high-recall triage component passes false positives to Qwen, the system encodes those as noise in the training data. This explains why most errors in Qwen consist of "pushed false positives". The triage schema only has a "No event" category, but we trained Qwen exclusively on positive examples. Hence, Qwen does not understand what a "no event" document looks like. As a result, it attempts to encode false positives as events even when none exist. Had we trained the triage component to also recognize "no event" cases, it could have filtered them out at the first stage without passing them to Qwen. While the current two-stage approach reduces the number of false positives passed to Qwen, we must still train Qwen to recognize and handle "hard negative" examples when possible.
...
Sliding Window Text Chunking: Right now, we chunk the text by taking 260 words at a time, with no overlap. This means if a document is long, that 260-word boundary cuts off events described across it, causing us to miss events or extract false information. To make this system more reliable for real-world use, we need to implement a sliding window chunker that takes tokens into account and has some overlap. For example, we could use a 250-token window with a 50-token overlap.

Multi-event document handling: Currently, the system outputs an array of extractions for every 260 words of text input. Thus, the system technically recognizes multiple events within a single document. However, we have no current means of recognizing that these multiple events actually represent parts of the same event occurring in different chunks of text. We need to add event de-duplication and temporal event linking as extensions for operational deployment.

FacilityHalt Boundary Hardening: The three categories FacilityHalt, SupplierInsolvency, and ForceMajeure are tricky to distinguish from each other. We should add training examples of situations that are physically identical but described differently using vocabulary associated with finance, contracts, or operations. This will help the model learn to differentiate these categories better.

Perplexity analysis as domain adaptation metric: Evaluating the fine-tuned model's perplexity on the hold-out supply chain corpus vs the general domain corpus (WikiText-103) would provide us with valuable additional insights. A decrease in perplexity on the supply chain domain—coupled with stable perplexity on the general WikiText-103 corpus—would demonstrate that LoRA successfully specializes the model without discarding its original foundational language capabilities.

Threshold calibration for DistilBERT classifier: Currently, we rely on the label distribution within the training data (60.6% of positive samples) to define the probability threshold for prediction. We would benefit from explicitly calibrating the classifier, for instance, using Platt scaling or isotonic regression, on a held-out validation set. This would allow us to adjust the precision-recall trade-off depending on the anticipated distribution of positive samples in the production data.

Force Majeure event classification nuances: We noted that current training data does not capture the true complexity of the ForceMajeure event type. The ForceMajeure label represents a general term that may pertain to any number of events beyond act-of-god or nature scenarios. We should think of ForceMajeure as an overarching category, under which specific causes such as wildfires or labor strikes fall. We can address this by introducing multi-label annotations or revising the event categorization.

Domain Generalization: To boost confidence in this two-stage, constrained-decoding architecture, we presented conceptual mappings in adjacent domains (Healthcare and Financial Markets) in Section 8. However, future work should proceed to fully implement and fine-tune these pipelines on real corpus data.

11. Conclusion

In this paper, which we devoted to the production-level engineering analysis of the two-stage supply chain event extraction pipeline powered by small language models, we obtained several valuable insights. However, the most important lesson of all remains the error-cost asymmetry of the two tasks, for which we devised particular data preparation strategies and training approaches to achieve satisfactory results.

First and foremost, in terms of the DistilBERT triage classifier, we learned that we had to set the positive training rate as high as 60.6% to induce a recall-biased decision boundary because of the substantially higher downstream cost associated with false negatives, highlighting the relative importance of early event detection. Then, while evidently different than triage, the extraction task required a significantly stronger balancing act to achieve equal extraction quality across all classes, which testified to the necessity of pursuing higher accuracy for lower-frequency events. We addressed this by training on a perfectly balanced dataset, which neutralized the FacilityHalt class's skew (which represented 30.7% of the 351-sample raw master set), leading to better precision for all classes, including FacilityHalt. Finally, we employed a constrained decoding layer, which transformed probabilistic schema compliance into deterministic decoding-layer constraints. Consequently, the overall F1 performance improved relatively by 47.9% (from 46.81% to 69.24%), and the schema validity increased from 23.33% to 100%, requiring training on only 1.196% of the base model's parameter budget. These error-cost driven considerations, which drove the final results, significantly impact the current research, and suggest several interesting directions for future exploration, including addressing timestamp hallucination contamination from few-shot prompting examples.

In our opinion, however, the more interesting direction for future research lies in exploring the phenomenon of type ambiguity, not at the schema-level, but at the event type level, which we can achieve by analyzing actual model outputs against gold annotations. In fact, we think a single sentence summarizes the key takeaway from this paper: researchers should always qualitatively analyze what the model interprets as evidence along with traditional F1 gains.